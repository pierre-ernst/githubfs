...
public class GitHubFileSystemProvider extends FileSystemProvider {
			...
			if (fileSystem == null) {
				if (create) {
					try {
						...
						throw (FileSystemNotFoundException) new FileSystemNotFoundException(schemeSpecificPart)
								.initCause(e);
					}
				} else {
	...
	public Path getPath(URI uri) {
		...
		if (i == -1) {
			throw new IllegalArgumentException(
					"URI: " + uri + " does not contain path info ex. github:apache/karaf#master!/");
		}
	...
	public InputStream newInputStream(Path path, OpenOption... options) throws IOException {
		...
		return ((GitHubPath) path).getFileSystem().newInputStream((GitHubPath) path);
	}
	...
	public DirectoryStream<Path> newDirectoryStream(Path dir, DirectoryStream.Filter<? super Path> filter)
		...
		return ((GitHubPath) dir).getFileSystem().newDirectoryStream((GitHubPath) dir, filter);
	}
	...
	public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs)
		...
		return ((GitHubPath) path).getFileSystem().newByteChannel(path);
	}
	...
	public <A extends BasicFileAttributes> A readAttributes(Path path, Class<A> type, LinkOption... options)
		...
		return ((GitHubPath) path).getFileSystem().readAttributes(path, type);
	}
